        ...
        if not meta_title or len(meta_title) < 30 or len(meta_title) > 60:
            ...
            if ai_title:
                fixes.append({
                    ...
                    "ai_reasoning": f"{'No meta title set — using product name as fallback.' if not meta_title else 'Meta title is ' + str(len(meta_title)) + ' chars — ' + ('too short' if len(meta_title) < 30 else 'too long') + '.'} An optimized title between 30-60 characters improves click-through rates from search results.",
                    "severity": "medium",
                    "category": "content",
                    "batch_id": batch_id,
                })
        ...
        if not meta_desc or len(meta_desc) < 70 or len(meta_desc) > 160:
            ...
            if ai_desc:
                fixes.append({
                    ...
                    "ai_reasoning": f"{'No meta description set.' if not meta_desc else 'Meta description is ' + str(len(meta_desc)) + ' chars.'} A compelling meta description between 120-155 characters increases click-through rates from search results.",
                    "severity": "medium",
                    "category": "content",
                    "batch_id": batch_id,
                })
