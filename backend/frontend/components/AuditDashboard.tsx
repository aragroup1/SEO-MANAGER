...
export default function AuditDashboard({ websiteId }: { websiteId: number }) {
  ...
  const [implementationStatus, setImplementationStatus] = useState<any>(null);
  ...
  const fetchLatestAudit = async () => {
    const response = await fetch(`/api/audits/${websiteId}/latest`);
    const data = await response.json();
    setAuditData(data);
  };

  const fetchAuditHistory = async () => {
    const response = await fetch(`/api/audits/${websiteId}/history?days=30`);
    const data = await response.json();
    setAuditHistory(data);
  };

  const fetchImplementationStatus = async () => {
    const response = await fetch(`/api/audits/${websiteId}/implementation-status`);
    const data = await response.json();
  ...
  const runAudit = async () => {
    ...
    await fetch(`/api/audits/${websiteId}/run`, { method: 'POST' });
    
    // Poll for completion
    const pollInterval = setInterval(async () => {
      await fetchLatestAudit();
      // Check if audit is complete (simplified)
      setTimeout(() => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="text-center">
            ...
            <div className="mt-4 flex items-center justify-center gap-2">
              ...
              <span className={auditData.audit.score_change > 0 ? 'text-green-400' : auditData.audit.score_change < 0 ? 'text-red-400' : 'text-gray-400'}>
          ...
          <div className="space-y-3">
            <h3 className="text-white font-medium mb-2">Implementation Tracking</h3>
            {implementationStatus && (
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-white font-bold">{implementationStatus.summary.verified}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-yellow-400 flex items-center gap-2">
                    <Clock className="w-4 h-4" />
                  ...
                  <span className="text-white font-bold">{implementationStatus.summary.pending}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-red-400 flex items-center gap-2">
                    <XCircle className="w-4 h-4" />
                  ...
                  <span className="text-white font-bold">{implementationStatus.summary.failed}</span>
                </div>
                <div className="mt-2 pt-2 border-t border-white/10">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-white font-bold">{auditData.audit.fixed_issues}</span>
                  </div>
                </div>
              </div>
            )}
      ...
      <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20">
        <div className="flex items-center gap-4">
          ...
          <div className="ml-auto text-white text-sm">
            Showing {filteredIssues.length} of {auditData.issues.length} issues
          </div>
        </div>
      </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h4 className="text-white font-medium flex items-center gap-2">
                        {issue.title}
                        {issue.implementation_status === 'verified' && (
                          <CheckCircle className="w-4 h-4 text-green-400" />
                        )}
                        {issue.implementation_status === 'in_progress' && (
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        <span className="text-xs text-gray-400">
                          Affects {issue.affected_pages.length} page{issue.affected_pages.length !== 1 ? 's' : ''}
                        </span>
                        <span className="text-xs text-gray-400">
                        ...
                        {issue.occurrences > 1 && (
                          <span className="text-xs text-orange-400">
                            Recurring ({issue.occurrences}x)
                          </span>
                        )}
                      </div>
                    </div>
                  </div>
                  <ChevronRight className={`w-5 h-5 text-gray-400 transition-transform ${expandedIssue === issue.id ? 'rotate-90' : ''}`} />
                </div>
              </div>
                        ...
                        <div className="space-y-1">
                          {issue.affected_pages.slice(0, 5).map((page, idx) => (
                            <div key={idx} className="flex items-center gap-2">
                              ...
                              <a href={page} target="_blank" rel="noopener noreferrer" className="text-purple-300 text-sm hover:text-purple-400 truncate">
                                {page}
                              </a>
                            </div>
                          ))}
          ...
          <div className="space-y-3">
            {auditData.recommendations.slice(0, 5).map((rec) => (
              <div key={rec.id} className="flex items-start gap-3 p-3 bg-white/5 rounded-lg">
                <span className="flex items-center justify-center w-8 h-8 bg-purple-500/20 text-purple-400 rounded-full font-bold text-sm">
                  {rec.priority}
                </span>
                <div className="flex-1">
